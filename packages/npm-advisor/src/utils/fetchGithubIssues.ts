...
export async function fetchGithubIssues(owner: string, repo: string) {
  ...
  const sampleUrl = `https://api.github.com/search/issues?q=${encodeURIComponent(sampleQuery)}&per_page=100`;
  ...
  const openCountUrl = `https://api.github.com/search/issues?q=${encodeURIComponent(openCountQuery)}&per_page=1`;
