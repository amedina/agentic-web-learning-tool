 ...
 */
import { mergeDeep } from "@agentic-labs/common";
...
const updateChromeStorage = async (
  ...
  let tableData: TablePersistentSettingsStoreContext["state"] = (data as any)?.[
    tabId + TABLE_PERSISTENT_SETTINGS_STORE_KEY
  ];
  ...
  if (!tableData) {
    (data as any)[tabId + TABLE_PERSISTENT_SETTINGS_STORE_KEY] = {};
    tableData = (data as any)[tabId + TABLE_PERSISTENT_SETTINGS_STORE_KEY];
  }
