 ...
 * CirclePieChart deliberately stays click-agnostic. When `interactive` is
 * false the circle renders as a plain div so it doesn't carry hover /
 * focus affordances for a click that wouldn't filter anything in.
 */
...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  // Filter tiles get an onClick only when their count > 0; clicking a
  // 0-count tile would set a filter that produces an empty list — a
  // dead-end interaction. The "Total Dependencies" tile is always
  // clickable because it acts as the "show all" reset.
  const matrixComponents: MatrixComponentProps[] = [
    ...
    },
    {
      ...
      onClick: breakdown.vulnerableCount > 0 ? handleTileClick : undefined,
    },
    {
      ...
      onClick: breakdown.licenseIssueCount > 0 ? handleTileClick : undefined,
    },
    {
      ...
      onClick: breakdown.replaceableCount > 0 ? handleTileClick : undefined,
    },
  ];
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl p-4">
        ...
        <div className="flex flex-nowrap items-start justify-center gap-3 max-w-[420px] mx-auto">
          ...
          <ClickableCircle
            title="With Vulnerabilities"
            onTrigger={handleTileClick}
            interactive={breakdown.vulnerableCount > 0}
          >
            <CirclePieChart
          ...
          <ClickableCircle
            title="License Issues"
            onTrigger={handleTileClick}
            interactive={breakdown.licenseIssueCount > 0}
          >
            <CirclePieChart
          ...
          <ClickableCircle
            title="Replaceable"
            onTrigger={handleTileClick}
            interactive={breakdown.replaceableCount > 0}
          >
            <CirclePieChart
