    ...
    try {
      if (window.LanguageDetector) {
        ...
        if (typeof window.ai.languageDetector.availability === "function") {
          ...
          typeof window.ai.languageDetector.capabilities === "function"
        ) {
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      if (sourceLang === "auto") {
        ...
        if (status === "no") {
          throw new Error(
            `Translation from ${finalSourceLang} to ${targetLang} is not supported.`,
          );
        }
        ...
        <div className="md:col-span-1 flex flex-col gap-4">
          <div className="bg-card border rounded-xl p-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Tabs
                value={mode}
                onValueChange={(v) => setMode(v as "detector" | "translator")}
                className="w-full"
              >
        ...
        <div className="md:col-span-2 flex flex-col gap-4 h-full">
              ...
              {mode === "detector" ? (
                ...
                >
                  {isDetecting ? (
                    ...
                    <>
                      <Languages className="w-4 h-4 mr-2" /> Execute Detector
                      API
                    </>
                  )}
                </Button>
                ...
                >
                  {isTranslating ? (
                    ...
                    <>
                      <ArrowRight className="w-4 h-4 mr-2" /> Execute Translator
                      API
                    </>
                  )}
                </Button>
          ...
          <div className="flex-[1.5] bg-card border rounded-xl p-4 flex flex-col gap-4 min-h-0 overflow-hidden">
            <div className="flex justify-between items-center border-b pb-2 flex-shrink-0">
              ...
              {((mode === "detector" && detectedResults.length > 0) ||
                (mode === "translator" && translateOutput)) && (
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() =>
                    copyToClipboard(
                      mode === "detector"
                        ? JSON.stringify(detectedResults, null, 2)
                        : translateOutput,
                    )
                  }
                  className="h-8"
