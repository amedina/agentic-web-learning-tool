...
export default function BuiltInAPIs() {
  ...
  const checkAPI = async (api: APIInfo) => {
    ...
    try {
      ...
      if (!globalObj) {
        ...
        setStatuses((prev) => ({
          ...prev,
          [api.id]: { status: "error", label, missingGlobal: true },
        }));
        return;
      }
    ...
    setStatuses((prev) => ({
      ...prev,
      [api.id]: { status, label, missingGlobal },
    }));
  };
  ...
  const downloadModel = async (apiId: string) => {
    ...
    try {
      ...
      setStatuses((prev) => ({
        ...prev,
        [apiId]: { ...prev[apiId], status: "error", label: "Download Failed" },
      }));
    } finally {
  ...
  return (
    <StatusCard
      title="Built-in AI APIs"
      description="Status of the Chrome AI primitives available in your browser."
      headerAction={
          ...
          <StatusItem
            ...
            action={
              info.status === "error" ? (
                ...
                !isDownloading &&
                info.label === "Needs Download" ? (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => downloadModel(api.id)}
