/**
 * External dependencies
 */
import type { PromptCommand } from '@google-awlt/design-system';
/**
 * Internal dependencies
 */
import type { SettingsState } from './types';

export const START_MCP_CONNECTION = 'START_MCP_CONNECTION';

export const INITIAL_PROVIDERS = [
  {
    id: 'browser-ai',
    name: 'Browser AI',
    models: [{ id: 'prompt-api', label: 'Prompt API' }],
  },
  {
    id: 'anthropic',
    name: 'Anthropic',
    models: [
      {
        id: 'claude-opus-4-5-20251101',
        label: 'Claude Opus 4.5',
        thinking: true,
      },
      {
        id: 'claude-haiku-4-5-20251001',
        label: 'Claude Haiku 4.5',
        thinking: true,
      },
      {
        id: 'claude-sonnet-4-5-20250929',
        label: 'Claude Sonnet 4.5',
        thinking: true,
      },
      {
        id: 'claude-opus-4-1-20250805',
        label: 'Claude Opus 4.1',
        thinking: true,
      },
      {
        id: 'claude-3-7-sonnet-20250219',
        label: 'Claude Sonnet 3.7',
        thinking: true,
      },
      { id: 'claude-opus-4-20250514', label: 'Claude Opus 4', thinking: true },
      {
        id: 'claude-3-5-haiku-20241022',
        label: 'Claude Haiku 3.5',
        thinking: true,
      },
      {
        id: 'claude-3-haiku-20240307',
        label: 'Claude Haiku 3',
        thinking: true,
      },
    ],
  },
  {
    id: 'open-ai',
    name: 'OpenAI',
    models: [
      { id: 'o1', label: 'o1', thinking: true },
      { id: 'o1-2024-12-17', label: 'o1-2024-12-17', thinking: true },
      { id: 'o3-mini', label: 'o3-mini', thinking: true },
      { id: 'o3-mini-2025-01-31', label: 'o3-mini-2025-01-31', thinking: true },
      { id: 'o3', label: 'o3', thinking: true },
      { id: 'o3-2025-04-16', label: 'o3-2025-04-16', thinking: true },
      { id: 'gpt-5', label: 'gpt-5', thinking: true },
      { id: 'gpt-5-2025-08-07', label: 'gpt-5-2025-08-07', thinking: true },
      { id: 'gpt-5-mini', label: 'gpt-5-mini', thinking: true },
      {
        id: 'gpt-5-mini-2025-08-07',
        label: 'gpt-5-mini-2025-08-07',
        thinking: true,
      },
      { id: 'gpt-5-nano', label: 'gpt-5-nano', thinking: true },
      {
        id: 'gpt-5-nano-2025-08-07',
        label: 'gpt-5-nano-2025-08-07',
        thinking: true,
      },
      { id: 'gpt-5-chat-latest', label: 'gpt-5-chat-latest' },
      { id: 'gpt-4.1', label: 'gpt-4.1' },
      { id: 'gpt-4.1-2025-04-14', label: 'gpt-4.1-2025-04-14' },
      { id: 'gpt-4.1-mini', label: 'gpt-4.1-mini' },
      { id: 'gpt-4.1-mini-2025-04-14', label: 'gpt-4.1-mini-2025-04-14' },
      { id: 'gpt-4.1-nano', label: 'gpt-4.1-nano' },
      { id: 'gpt-4.1-nano-2025-04-14', label: 'gpt-4.1-nano-2025-04-14' },
      { id: 'gpt-4o', label: 'gpt-4o' },
      { id: 'gpt-4o-2024-05-13', label: 'gpt-4o-2024-05-13' },
      { id: 'gpt-4o-2024-08-06', label: 'gpt-4o-2024-08-06' },
      { id: 'gpt-4o-2024-11-20', label: 'gpt-4o-2024-11-20' },
      { id: 'gpt-4o-mini', label: 'gpt-4o-mini' },
      { id: 'gpt-4o-mini-2024-07-18', label: 'gpt-4o-mini-2024-07-18' },
      { id: 'gpt-4-turbo', label: 'gpt-4-turbo' },
      { id: 'gpt-4-turbo-2024-04-09', label: 'gpt-4-turbo-2024-04-09' },
      { id: 'gpt-4', label: 'gpt-4' },
      { id: 'gpt-4-0613', label: 'gpt-4-0613' },
      { id: 'gpt-3.5-turbo-0125', label: 'gpt-3.5-turbo-0125' },
      { id: 'gpt-3.5-turbo', label: 'gpt-3.5-turbo' },
      { id: 'gpt-3.5-turbo-1106', label: 'gpt-3.5-turbo-1106' },
      { id: 'chatgpt-4o-latest', label: 'chatgpt-4o-latest' },
    ],
  },
  {
    id: 'gemini',
    name: 'Gemini',
    models: [
      { id: 'gemini-3-pro-preview', label: 'gemini-3-pro-preview', thinking: true },
      { id: 'gemini-3-flash-preview', label: 'gemini-3-flash-preview', thinking: true },
      { id: 'gemini-2.5-pro', label: 'gemini-2.5-pro', thinking: true },
      { id: 'gemini-2.5-flash', label: 'gemini-2.5-flash', thinking: true },
      {
        id: 'gemini-2.5-flash-lite',
        label: 'gemini-2.5-flash-lite',
        thinking: true,
      },
      {
        id: 'gemini-2.5-flash-lite-preview-06-17',
        label: 'gemini-2.5-flash-lite-preview-06-17',
        thinking: true,
      },
      { id: 'gemini-2.0-flash', label: 'gemini-2.0-flash', thinking: true },
      { id: 'gemini-1.5-pro', label: 'gemini-1.5-pro' },
      { id: 'gemini-1.5-pro-latest', label: 'gemini-1.5-pro-latest' },
      { id: 'gemini-1.5-flash', label: 'gemini-1.5-flash' },
      { id: 'gemini-1.5-flash-latest', label: 'gemini-1.5-flash-latest' },
      { id: 'gemini-1.5-flash-8b', label: 'gemini-1.5-flash-8b' },
      { id: 'gemini-1.5-flash-8b-latest', label: 'gemini-1.5-flash-8b-latest' },
      { id: 'gemma-3-27b-it', label: 'gemma-3-27b-it' },
    ],
  },
];

export const BUILT_IN_COMMANDS: PromptCommand[] = [
  {
    name: 'fix-bugs',
    description: 'Analyze code for bugs and propose fixes.',
    instructions:
      'Analyze the following code for bugs or potential issues and provide a fixed version:\n\n$ARGUMENTS',
    isBuiltIn: true,
    enabled: true,
    sendToLLM: true,
  },
  {
    name: 'explain-code',
    description: 'Explain code step-by-step.',
    instructions:
      'Explain how the following code works step-by-step:\n\n$ARGUMENTS',
    isBuiltIn: true,
    enabled: true,
    sendToLLM: true,
  },
  {
    name: 'settings',
    description: 'Open the settings page',
    instructions: ' ',
    isBuiltIn: true,
    enabled: true,
    sendToLLM: false,
  },
  {
    name: 'clear',
    description: 'Clears the chat history.',
    instructions: ' ',
    isBuiltIn: true,
    enabled: true,
    sendToLLM: false,
  },
  {
    name: 'help',
    description: 'Show the available list of commands.',
    instructions: ' ',
    isBuiltIn: true,
    enabled: true,
    sendToLLM: false,
  },
  {
    name: 'optimize',
    description: 'Optimize code for performance/readability.',
    instructions:
      'Optimize the following code for performance and readability:\n\n$ARGUMENTS',
    isBuiltIn: true,
    enabled: true,
    sendToLLM: true,
  },
  {
    name: 'generate-tests',
    description: 'Write unit tests for code.',
    instructions:
      'Write comprehensive unit tests for the following code using preferred testing framework:\n\n$ARGUMENTS',
    isBuiltIn: true,
    enabled: true,
    sendToLLM: true,
  },
];

export const DEFAULT_SETTINGS: SettingsState = {
  theme: 'auto',
  logLevel: 'SILENT',
};

export const EXPORT_JSON_VERSION = '1.0';
