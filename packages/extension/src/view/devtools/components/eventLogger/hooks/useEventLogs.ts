...
export const useEventLogs = (initialLastRunToolName: string | null = null) => {
  ...
  const [lastRunToolName, setLastRunToolName] = useState<string | null>(
    initialLastRunToolName
  );
  ...
  useEffect(() => {
    if (initialLastRunToolName) {
      setLastRunToolName(initialLastRunToolName);
    }
  }, [initialLastRunToolName]);
  ...
  return {
    ...
    selectedKey,
    setSelectedKey,
    setLastRunToolName, // Expose this if we need to set it manually from outside
  };
};
