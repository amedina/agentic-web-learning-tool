...
class ChromeAILanguageModel {
	...
	async doGenerate(callOptions: LanguageModelV2CallOptions) {
		...
		const { toolCall: toolCalls, textPrefix: textContent } =
		...
		return {
			stream: new ReadableStream({
				start: async (controller) => {
					...
					let isAborted = false;
					let activeStreamReader: ReadableStreamDefaultReader<string> | null =
						null;
					...
					if (this.formattedTools.length === 0) {
						emitTextDelta(
							'Error: No tools are configured for tool calling.'
						);
						finishStream('error');
						return;
					}
					...
					try {
						...
						while (
							...
							while (!isAborted && activeStreamReader) {
								...
								while (fenceScanner.hasContent()) {
									...
									const scanResult =
										fenceScanner.detectStreamingFence();
									...
									if (scanResult.completeFence) {
										...
										if (
											...
											const allArgs =
												extractArguments(rawToolBuffer);
											if (
												allArgs.length >
												processedArgLength
											) {
										...
										if (
											...
											parsedCalls[0].toolCallId =
												currentToolCallId;
										}

										toolCallsFound = parsedCalls;
										hasToolCalls =
											toolCallsFound.length > 0;

										// Emit final Tool Call events
										for (const [
											idx,
											tool,
										] of toolCallsFound.entries()) {
											const callId: string =
												idx === 0 && currentToolCallId
													? currentToolCallId
													: tool.toolCallId;
						...
						if (callOptions.abortSignal) {
							callOptions.abortSignal.removeEventListener(
								'abort',
								handleAbort
							);
						}
