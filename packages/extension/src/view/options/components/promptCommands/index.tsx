...
export function PromptCommandsTab() {
	...
	const [builtInCommands, setBuiltInCommands] =
		useState<PromptCommand[]>(BUILT_IN_COMMANDS);
		...
		chrome.storage.local.get(
			...
			(result) => {
				if (
					...
					Array.isArray(result.promptCommands)
				) {
					setUserCommands(result.promptCommands);
				}

				if (
					result.builtInPromptCommands &&
					Array.isArray(result.builtInPromptCommands)
				) {
					const stored =
