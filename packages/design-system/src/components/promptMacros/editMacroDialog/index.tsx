...
interface EditMacroDialogProps {
    ...
    macro?: PromptMacro;
    onSave: (macro: PromptMacro) => void;
    onDelete?: (macro: PromptMacro) => void;
    existingNames?: string[];
}

export function EditMacroDialog({ open, onOpenChange, macro, onSave, onDelete, existingNames = [] }: EditMacroDialogProps) {
    ...
    useEffect(() => {
        if (open) {
            ...
            setInstructions(macro ? macro.instructions : '');
            setError('');
        }
    }, [open, macro]);
    ...
    const handleSave = () => {
        ...
        if (!/^[a-zA-Z0-9_\-]+$/.test(trimmedName)) {
            setError('Letters, numbers, hyphens, and underscores only. No slashes or spaces.');
            return;
        }
    ...
    const exampleOutput = instructions
        ? (instructions.includes('$ARGUMENTS')
            ? instructions.replace('$ARGUMENTS', 'your arguments here')
            : instructions)
        : "Your template here";
                ...
                <Dialog.Content className="fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-[600px] max-w-[95vw] bg-white text-gray-900 border border-gray-200 rounded-xl shadow-2xl z-50 flex flex-col p-6 outline-none">
                    ...
                    <div className="flex flex-col gap-5">
                        ...
                        <div className="mt-2 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                            <label className="text-sm font-bold text-gray-800 mb-2 block">Example Usage</label>
                            <div className="grid grid-cols-[auto,1fr] gap-x-4 gap-y-2 text-sm items-baseline">
                                <span className="font-semibold text-gray-600 text-right min-w-[70px]">Input:</span>
                                <code className="bg-white border border-gray-200 px-2 py-1.5 rounded text-blue-600 font-mono block w-full">{exampleInput}</code>

                                <span className="font-semibold text-gray-600 text-right min-w-[70px]">Expands to:</span>
                        ...
                        <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-100">
                            ...
                            <Button onClick={handleSave} className="bg-blue-600 hover:bg-blue-700 text-white gap-2 shadow-sm">
