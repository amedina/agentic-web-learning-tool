...
- **Activity Bar side panel** — a dedicated NPM Advisor view with three tabs:
  - **Dependencies** — every dep in the active `package.json` with score, bundle size, advisory count, and a per-row drill-down.
  ...
  - **Ask AI** — a chat over the active project that can answer questions about a dependency's security posture, alternatives, and replacement codemods.
...
- **NPM Advisor: Set up MCP server for AI clients (Claude Code, Cursor, …)** — opens a one-click setup webview that installs the npm-advisor MCP server into the AI clients you already have on this machine and skips the ones you don't. Companion **NPM Advisor: Remove MCP server from AI clients** undoes it.
- **NPM Advisor: Sign in to GitHub (lifts API rate limit)** — authenticates with GitHub via VSCode's built-in OAuth flow so analyzer-core's GitHub fetches use the user's 5 000-req/hr quota instead of the 60-req/hr anonymous one. Companion **Disconnect GitHub session** drops the credential.
