...
import type { SelectionToolConfig } from "@agentic-web-labs/engine-core";
...
const ToolNode = () => {
  ...
  const config = useMemo(() => {
    ...
    const _config = node.config as SelectionToolConfig;

    return {
      title: _config.title || "Selection Tool",
      type: node?.type,
  ...
  return (
    <ToolNodeContainer
      title={config?.title || "Selection Tool"}
      Icon={MousePointer2}
      type={config?.type || ""}
      ...
      <div className="h-fit w-full flex flex-col relative px-2">
        <div className="w-full bg-slate-50 dark:bg-zinc-800 rounded-md px-3 py-2 my-4 border border-slate-100 dark:border-border">
          ...
          <p className="truncate text-xs text-slate-600 dark:text-zinc-400 italic font-medium">
            {config?.description || "Selects text from the page"}
          </p>
        </div>
        <Handle
          type="target"
